...
// use network_algorithms::minimum_cost_flow::{BlockSearchPivotRule, MinimumCostFlowSolver};
use rstest::rstest;
use std::fs::read_to_string;
use std::path::PathBuf;
use network_algorithms::algorithms::minimum_cost_flow::status::Status;
use std::fmt::Debug;
use network_algorithms::algorithms::minimum_cost_flow::edge::MinimumCostFlowEdge;
use network_algorithms::algorithms::minimum_cost_flow::node::MinimumCostFlowNode;
...
fn minimum_cost_flow(#[files("tests/minimum_cost_flow/*/*.txt")] path: PathBuf, #[case] solver: Solver) {
    ...
    let mut graph: Graph<Directed, MinimumCostFlowNode<i128>, MinimumCostFlowEdge<i128>> = Graph::new_directed();
    let mut nodes = Vec::new();

    read_to_string(&path).unwrap().split('\n').enumerate().for_each(|(i, line)| {
        ...
        if i == 0 {
            ...
            graph.add_edge(nodes[from], nodes[to], MinimumCostFlowEdge{lower, upper, cost});
        }
    });
