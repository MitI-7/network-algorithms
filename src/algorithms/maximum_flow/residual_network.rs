    ...
    F: FlowNum,
{
    pub fn build(&mut self, graph: &Graph<Directed, N, MaximumFlowEdge<F>>) {
        ...
        for (edge_index, e) in graph.edges().enumerate() {
            ...
            self.flow[arc_id_u.index()] = F::zero();
            self.upper[arc_id_u.index()] = e.data.upper;
            self.rev[arc_id_u.index()] = arc_id_v;

            // v -> u
            self.to[arc_id_v.index()] = u;
            self.flow[arc_id_v.index()] = e.data.upper;
            self.upper[arc_id_v.index()] = e.data.upper;
    ...
    #[inline]
    pub fn push_flow(&mut self, u: NodeId, arc_id: ArcId, flow: F, without_excess: bool) {
        self.flow[arc_id.index()] += flow;
        self.flow[self.rev[arc_id.index()].index()] -= flow;
    ...
    pub fn update_distances_to_sink(&mut self, source: NodeId, sink: NodeId) {
        self.que.clear();
