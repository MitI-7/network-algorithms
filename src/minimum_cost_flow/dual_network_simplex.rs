...
pub struct DualNetworkSimplex<'a, Flow> {
    st: &'a mut SpanningTreeStructure<Flow>,
    // source: usize,
    sink: usize,
}

impl<'a, Flow> DualNetworkSimplex<'a, Flow>
where
    Flow: NumAssign + Neg<Output = Flow> + Ord + Copy + std::fmt::Display,
{
    ...
    fn calculate_violation(edge: &InternalEdge<Flow>, _st: &SpanningTreeStructure<Flow>) -> Flow {
