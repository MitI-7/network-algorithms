...
pub trait MinimumCostFlowNum:
    Zero + Ord + Add<Output = Self> + Sub<Output = Self> + Neg<Output = Self> + Mul<Output = Self> + AddAssign + SubAssign + Copy + One + Debug
{
}
impl<T> MinimumCostFlowNum for T where
    T: Zero + Ord + Add<Output = Self> + Sub<Output = Self> + Neg<Output = Self> + Mul<Output = Self> + AddAssign + SubAssign + Copy + One + Debug
{
}
