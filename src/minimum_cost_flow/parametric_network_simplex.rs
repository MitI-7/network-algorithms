...
pub struct ParametricNetworkSimplex<'a, Flow> {
    st: &'a mut SpanningTreeStructure<Flow>,
    sink: usize,
}

impl<'a, Flow> ParametricNetworkSimplex<'a, Flow>
where
    Flow: NumAssign + Neg<Output = Flow> + Ord + Copy,
{
    ...
    pub fn solve(&mut self) -> Status {
        ...
        let (source, sink, artificial_nodes, artificial_edges) = self.st.construct_extend_network_one_supply_one_demand();
    ...
    pub fn solve_max_flow_with_min_cost(&mut self, source: usize, sink: usize) -> (Status, Flow) {
    ...
    pub(crate) fn select_leaving_edge(&self) -> Option<(usize, Flow)> {
